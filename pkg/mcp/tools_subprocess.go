...
// runInWorkdir runs cmd with args in the session's working directory,
// combining stdout and stderr into a single tool result.
//
...
func runInWorkdir(ctx context.Context, c *Claims, cmd string, args ...string) (*mcplib.CallToolResult, error) {
	if c.Workdir == "" {
