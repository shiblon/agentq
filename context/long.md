...
## Deployment architecture (settled 2026-05-16)

### Three independent pod types -- no correlated lifecycles

**AgentQ pod** (worker + eqlink)
- worker container: claims sessions from EntroQ, issues signed JWT, puts task+JWT onto runner queue
- eqlink container: outbound only -- no direct external egress needed
- AgentQ never speaks directly to MCP or Runner

**Runner pod** (runner microservice + eqlink)
- eqlink container: inbound -- claims tasks from runner queue
- runner container: receives JWT in task payload, contacts MCP service (presents JWT at SSE handshake),
  runs AI agent (Claude CLI or other)
- Needs external egress for LLM API; can reach MCP service; nothing else
- Posts result back to EntroQ queue when done

**MCP pod** (Deployment + Service, pool)
- Stateless pool -- all instances alike
- Auto-configures on first session interaction via JWT (no separate admin/config call)
- JWT carries: allowed tool list, volume to use, working directory to expose, session ID, expiry, issuer
- MCP validates JWT at SSE handshake, stores allowlist as session state, filters tools/list response
- Well-behaved agent assumption: tools/list is the main enforcement point; per-call check is server-side allowlist lookup
- Scales independently via standard k8s HPA (connection count / request rate)
...
MCP session ID (protocol-level correlation) is separate from this JWT.
...
Prior: multi-container pod with AgentQ+MCP+Runner sharing network namespace (MCP egress bled to AgentQ).
Current: three separate pod types, proper network isolation via NetworkPolicy, independent scaling.

## worker-pod-architecture
## Worker Architecture -- Settled Decisions (2026-05-16)

### Component roles and lifecycles
- **AgentQ**: long-lived. Claims from outer EntroQ (ClaimWithRenew), configures MCP,
  dispatches to runner, collects delta, posts result. No system tools. No internet egress.
- **MCP service**: task-scoped lifecycle (idle -> active -> idle). Two ports:
  - Port X (admin): configure/reset tool list. Requires AgentQ JWT. Agent never sees this port.
  - Port Y (SSE): tool endpoint for agent. Open, no auth.
  Configured by AgentQ before runner starts. Needs internet egress for tools (git, web, etc.).
- **Runner (agent)**: task-scoped lifecycle. Runs claude with --mcp-config pointing at Port Y.
  Cleared environment -- no credentials. Can reach Port Y only, not Port X. No internet egress.

### Tool list
- Carried in the task payload. Set by supervisor or intent agent. AgentQ reads it, passes to MCP
  via Port X (with JWT). AgentQ makes no decisions about which tools to include.

### Auth
- AgentQ uses its existing machine user JWT to authenticate admin calls to MCP Port X.
- MCP verifies via IDP public key -- no shared secret needed.
- Runner never sees AgentQ's JWT.

### NOT YET SETTLED
- Deployment topology: single pod (3 containers) vs. multi-pod vs. StatefulSet.
  Key tension: single pod = shared network namespace (MCP egress bleeds to AgentQ);
  multi-pod = proper NetworkPolicy isolation but needs ReadWriteMany volume + cross-pod comms.
- Volume strategy: ReadWriteMany (NFS) vs. ReadWriteOnce per paired components.
- How runner communicates results back: direct HTTP, inner queue, or EntroQ runner queue.

## supervisor-mental-model
## Supervisor Mental Model (settled 2026-05-16)
...
- AgentQ is a mechanical relay: reads task, configures MCP, runs agent, collects delta. No decisions.
...
## Standard deployment: queue-native workers + tool pod

Workers are queue-native Go programs using the EntroQ client directly (no eqlink,
no sidecar). Each worker is the 'AgentQ pod' in the deployment sense.

Per-task flow:
  AgentQ worker (Go, EntroQ client)
    - claims task from queue
    - loads session, resolves permissions + scoped token
    - starts MCP server (pkg/mcp) with tools allowed for this invocation
    - invokes tool pod (LLM agent) with only:
        * MCP server URL (srv.URL()+"/sse")
        * LLM API endpoint egress
    - waits for agent exit, collects artifacts
    - re-enqueues result to supervisor, shuts down MCP server

Tool pod:
  - LLM agent (e.g. Claude CLI)
  - No shell, no filesystem, no arbitrary network
  - NetworkPolicy: only MCP server + LLM API
  - gVisor optional for kernel isolation

## Token passing

Scoped token passed in for now. Future: MCP server mints downscoped tokens,
agent never sees underlying credentials. Tool interface is stable across both.

## Eqlink (deferred)

eqlink could be refactored into a library for workers to embed queue mechanics
directly as a package. Deferred; current approach uses EntroQ Go client directly.

eqlink sidecar model was considered and rejected: we're not trying to be a mesh,
we think in terms of workflow queues. Queue-native is simpler and the chokepoint
is unambiguous.

## project_mcp_sandbox
## MCP as the agent sandbox

The sandboxing strategy for AgentQ: agents run in locked-down tool pods with
NO shell, filesystem, or network access except:
  1. The LLM API endpoint (e.g. Anthropic API)
  2. An MCP server running in the AgentQ worker pod

The MCP server is the capability enforcer. It is started fresh per task invocation,
registered with exactly the tools that session/invocation is permitted to use,
and shut down when the task completes. The agent can only do what MCP exposes.
...
## Typical worker usage pattern

    sink, artifacts := mcp.CollectingSink()
    tools := mcp.SessionTools(session, agentName, sink)
    srv, err := mcp.New(tools)
    defer srv.Close(ctx)
    // invoke agent with srv.URL()+"/sse" as MCP endpoint
    // after agent exits, *artifacts holds what was written

## Deferred: eqlink as a library

eqlink could be refactored into a package (pkg) with a thin binary wrapper,
allowing AgentQ workers to embed queue mechanics directly. Currently deferred
in favor of building with queue-native Go workers (direct EntroQ client) first.
No eqlink sidecar in the current design.
